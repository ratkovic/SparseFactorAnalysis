...
##2) Converting a matrix to the Z scale
	...
	wordprob.mat<-voteprob.mat<-NULL
			...
			mean.bin<-sigma^.5*Theta.last[row.type=="bin",][votes.mat[row.type=="bin",]==1]
			Z.next[row.type=="bin",][votes.mat[row.type=="bin",]==1]<-
			#rtn(.mean=mean.bin,	.low=0*mean.bin)
			#rtn(.mean = rep(0, 1), .sd = rep(1, length(.mean)), .low = rep(-Inf,
#  length(.mean)), .high = rep(Inf, length(.mean)), .checks = TRUE)
			rtruncnorm(sum(votes.mat[row.type=="bin",]==1), mean=sigma^.5*Theta.last[row.type=="bin",][votes.mat[row.type=="bin",]==1], a=0, sd=sigma^.5)
			mean.bin<-sigma^.5*Theta.last[row.type=="bin",][votes.mat[row.type=="bin",]==0]
		Z.next[row.type=="bin",][votes.mat[row.type=="bin",]==0]<-
#rtn(.mean=mean.bin,	.high=0*mean.bin)
		rtruncnorm(sum(votes.mat[row.type=="bin",]==0), mean=sigma^.5*Theta.last[row.type=="bin",][votes.mat[row.type=="bin",]==0], b=0 , sd=sigma^.5)
...
#  length(.mean)), .high = rep(Inf, length(.mean)), .checks = TRUE)
			...
			#rtn(.mean = rep(0, 1), .sd = rep(1, length(.mean)), .low = rep(-Inf,
#  length(.mean)), .high = rep(Inf, length(.mean)), .checks = TRUE)
		mean.count<-as.vector(Theta.last[row.type=="count",])
		missing.count<-as.vector(missing.mat[row.type=="count",])
		a1<-as.vector(taus[votes.mat[row.type=="count",]+1 ])
		b1<-as.vector(taus[votes.mat[row.type=="count",]+2 ] )
		Z.next.temp<-#rtn(.mean=mean.count,.low=a1,.high=b1 )
		rtruncnorm(num.sample.count, mean=Theta.last[row.type=="count",], a=taus[votes.mat[row.type=="count",]+1 ],b=taus[votes.mat[row.type=="count",]+2 ] )

		Z.next.temp[missing.count==1]<-rnorm(sum(missing.count==1),mean.count[missing.count])
